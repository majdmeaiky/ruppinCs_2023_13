...
namespace Job_Scheduling
{
    class Program
    {
        static List<int[,]> conformingToTheConsraintList;
        static int NumOfWorkers = 12;
        static int Size = 7* NumOfWorkers;
        static int Prop;
        static int Counter_Prop;
        static string filePath;
        static StreamWriter writer = new StreamWriter("../../output9.txt");
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 50; i++)
            {
                ...
                Prop = 1;
                Counter_Prop = 1;
                conformingToTheConsraintList = new List<int[,]>();
                FairSchedulingAlgorithm(i);
            }
        ...
        static void InitializeFromFile(string filePath)
        {
            ...
            foreach (string line in lines)
            {
                ...
                int[,] array = new int[numbers.Length, numbers.Length];

                // Parse and assign the values to the array
                for (int i = 0; i < numbers.Length; i++)
                {
                    array[0, i] = int.Parse(numbers[i]);
                }

                // Add the array to the temporary list
                tempList.Add(array);
            }

            // Assign the temporary list to the variable
            conformingToTheConsraintList = tempList;
        }

        public static void FairSchedulingAlgorithm(int filename)
        {
            Random rand = new Random();
            int[,] EmploysRequestsMatrix=new int[Size, Size];
            string fileContent = File.ReadAllText("../../input/"+filename+".txt");
            string[] numbers = fileContent.Replace("{", "").Replace("}", "").Replace(",", "").Split();

            // Iterate over the numbers and assign them to the matrix
            int spaces_index = 0;
            for (int i = 0; i < Size; i++)
            {
                ...
                {
                    if ( numbers[spaces_index] != "")
                    {
                        EmploysRequestsMatrix[i, j] = int.Parse(numbers[spaces_index]);
                    }
                    else
                    {
                        j--;
        ...
        public static int[] RunHungarianOnAllMatrices()
        {
            ...
            writer.WriteLine(chossen_matrix_val);
        ...
        public static void ShowWeeklySchedule(int [] chossen_matrix)
        {
            ...
            for (int i = 0; i < chossen_matrix.Length; i++)
            {
                ...
                if (chossen_matrix[i] < 21)
                {
                    ...
                    {
                        m[chossen_matrix[i] / 3] = "Worker" + (worker_index-1);

                    }
                    if (chossen_matrix[i] % 3 == 1)
                    {
                        e[chossen_matrix[i] / 3] = "Worker" + (worker_index - 1);

                    }
                    if (chossen_matrix[i] % 3 == 2)
                    {
                        n[chossen_matrix[i] / 3] = "Worker" + (worker_index - 1);
            ...
            for (int i = ii; i < Size - 1; i++)
            {
                ...
                if (m[i, (i * 3 + 2) % 21] != int.MaxValue && m[i + 1, (i * 3 + 2) % 21 + 1] != int.MaxValue)
                {
                    ...
                    if (action == 1)
                    {
                        ...
                        if (m[i, (i * 3 + 2) % 21] == 1) //night priority is 1
                        {
                        ...
                        else if (m[i + 1, (i * 3 + 2) % 21 + 1] == 1) //morning priority is 1
                        {
                            Create_Conforming_to_the_constraint_matrices(a, i + 1, (i * 3 + 2) % 21 + 3);
                            Create_Conforming_to_the_constraint_matrices(b, i + 1, (i * 3 + 2) % 21 + 3);
                        }
                        else //both morning and night collosion not equal 1
                        {
                            if (Counter_Prop % 2 == 0)
                            {
                                Create_Conforming_to_the_constraint_matrices(a, i + 1, (i * 3 + 2) % 21 + 3);
